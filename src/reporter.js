...
Reporter.prototype.logSelectedFiles = function (contracts, tests) {
  ...
  console.log();
  if (solmutatorConfig.testingFramework === "custom" && solmutatorConfig.skipTests.length > 0) {
...
Reporter.prototype.logLookup = function (mutations, genTime, operators) {
  ...
  const lookup = mutations.length + " mutation(s) found in " + genTime + " seconds \n" +
    "Generated mutations saved to solmutator/results/results.csv and solmutator/results/mutations.json \n"
...
Reporter.prototype.mutantStatus = function (mutant) {
  switch (mutant.status) {
    ...
    case "timedout":
      ...
      console.log(
        "Mutant " + this.chalkMutant(mutant) + " has timed out after " + (solmutatorConfig.testingTimeOutInSec) / 60 + " minutes"
      );
      break;
    case "redundant":
...
Reporter.prototype.logAndSaveTestSummary = function (time) {
  ...
  console.log(
    "solmutator generated " + totalMutants + " mutants: \n" +
