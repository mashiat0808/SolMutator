...
function setup(callback) {
  ...
  //Setup .solmutator dir
  if (!fs.existsSync(solmutatorDir)) {
    fs.mkdirSync(solmutatorDir);
  }
  utils.setupResultsDir();

  //Get configurable directories from the solmutator-config file
...
function contractSelection(files) {
  ...
  for (const file of files) {
    ...
    for (const relCPath of solmutatorConfig.skipContracts) {
...
function testSelection(files) {
  ...
  for (const file of files) {
    let skipTest = false;
    for (const relTPath of solmutatorConfig.skipTests) {
      let absolTPath = utils.getTestDir() + "/" + relTPath;
      if (file.startsWith(absolTPath) && relTPath !== "" && solmutatorConfig.testingFramework !== "custom") {
        skipTest = true;
        break;
      }
    }
    if (solmutatorConfig.testingFramework === "forge" && !file.endsWith(".t.sol")) {
      skipTest = true;
    }
    if (!skipTest) {
