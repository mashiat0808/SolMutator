...
const config = {
  solmutatorDir: rootDir + "/solmutator",
  mutantsDir: rootDir + "/solmutator/results/mutants",
  reportTxt: rootDir + "/solmutator/results/solmutator-log.txt",
  resultsDir: rootDir + "/solmutator/results",
  baselineDir: rootDir + "/solmutator/baseline",
  mutOpsConfig: rootDir + "/src/operators.config.json",
  contractsGlob: '/**/*.sol',
  ...
  if (fs.existsSync(config.baselineDir)) {
    ...
    glob(config.baselineDir + '/contracts' + config.contractsGlob, (err, files) => {
      ...
      for (const file of files) {
        let relativeFilePath = file.split("solmutator/baseline/contracts")[1];
    ...
    glob(config.baselineDir + '/test' + config.testsGlob, (err, files) => {
      ...
      for (const file of files) {
        let relativeFilePath = file.split("solmutator/baseline/test")[1];
        let fileDir = path.dirname(relativeFilePath);
...
function getContractsDir() {
  const validContractsDirs = [
    solmutatorConfig.contractsDir && solmutatorConfig.contractsDir !== '' ? "/" + solmutatorConfig.contractsDir : null,
    "/contracts",
    "/src"
  ];
...
function getTestDir() {
  const validTestDirs = [
    solmutatorConfig.testDir && solmutatorConfig.testDir !== '' ? "/" + solmutatorConfig.testDir : null,
    "/test",
    "/tests"
  ];
...
function getBuildDir() {
  const validBuildDirs = [
    solmutatorConfig.buildDir && solmutatorConfig.buildDir !== '' ? "/" + solmutatorConfig.buildDir : null,
  ...
  if (foundDir) {
    ...
    console.error(chalk.red("Error: No valid build directory found in " + rootDir + ".\nPlease compile your contracts and/or specify a build directory in your solmutator-config.js"));
    process.exit(1);
  }
}
